...
#' computes cor correlations, performs bootstrapping to calculate p-values,
...
Generate_edgelist <- function(filename, bootstraps = 1000, p_value_threshold = 0.05) {
  ...
  permute_w_replacement <- function(frame) {
    s <- nrow(frame)
    perm <- as.data.frame(lapply(frame, function(x) sample(x, size = s, replace = TRUE)))
    rownames(perm) <- rownames(frame)
    return(perm)
  }
  ...
  bootstrap_correlations <- function(df, cor, samplename, bootstraps) {
    ...
    for (i in 1:bootstraps) {
      ...
      perm_cor <- sampling_cor(bootstrap, samplename)
  ...
  # Define the sampling cor function
  sampling_cor <- function(frame_ori, samplename) {
  ...
  # Compute cor correlations
  Framecor <- sampling_cor(comb_table_clr, sample_name)
  ...
  cor_corfilter <- Framecor
  cor_corfilter[perm_pval > p_value_threshold] <- NA
  
  # Generate the edge list
  cor_corfilter_melt <- melt(as.matrix(cor_corfilter), na.rm = TRUE)
  colnames(cor_corfilter_melt) <- c('Node1', 'Node2', 'Correlation')
  
  # Remove self-loops if any
  cor_corfilter_melt <- cor_corfilter_melt[cor_corfilter_melt$Node1 != cor_corfilter_melt$Node2, ]
  
  # Remove duplicate edges (since the correlation matrix is symmetrical)
  cor_corfilter_melt <- cor_corfilter_melt[!duplicated(t(apply(cor_corfilter_melt[, 1:2], 1, sort))), ]
  
  # Return the edge list
  return(cor_corfilter_melt)
}
