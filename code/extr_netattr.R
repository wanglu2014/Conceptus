...
#' @export
extr_netattr <-
function(mypath = "../data/", phylo_list = c('example_phyloseq.Rdata')) {
  # Note: This function requires phyloseq .Rdata files as input.
  # Example data not included due to size. See Supplementary Methods for data sources.
  library(data.table)
  library(dplyr)
  library(igraph)
  library(reshape2)
  ...
  for (phylo_name in phylo_list) {
    ...
    # Extract phylotxt from filename
